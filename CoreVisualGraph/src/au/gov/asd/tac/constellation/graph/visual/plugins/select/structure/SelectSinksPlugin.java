...
/**
 * Select all nodes with an incoming transaction, but no outgoing transactions
 *
 * @author aquila
 */
@ServiceProvider(service = Plugin.class)
@NbBundle.Messages("SelectSinksPlugin=Add to Selection: Sinks")
public class SelectSinksPlugin extends SimpleEditPlugin {
    ...
    public void edit(final GraphWriteMethods graph, final PluginInteraction interaction, final PluginParameters parameters) throws InterruptedException {
        ...
        if (vxSelected != Graph.NOT_FOUND) {
            ...
            for (int position = 0; position < vertexCount; position++) {
                //Convert the position to a transaction
                ...
                //Set the Nodes that are sinks to be selected
