...
/**
 * Merge nodes based on the identifier prefix length
 *
 * @author arcturus
 */
...
public class MergeNodesByPrefix implements MergeNodeType {
    ...
    public void updateParameters(final Map<String, PluginParameter<?>> parameters) {
        ...
        parameters.get(THRESHOLD_PARAMETER_ID).setDescription("The prefix length to consider");
    ...
    public final Map<Integer, Set<Integer>> getNodesToMerge(final GraphWriteMethods graph, final Comparator<String> leadVertexChooser, final int threshold, final boolean selectedOnly) {
        ...
        if (identifierAttribute != Graph.NOT_FOUND) {
            ...
            for (int vertexPosition = 0; vertexPosition < vertexCount; vertexPosition++) {
                ...
                if (identifier.length() >= threshold && (!selectedOnly || graph.getBooleanValue(selectedAttribute, vertexId))) {
                    ...
                    matchingVertices.put(vertexId, identifier);
                }
            }
