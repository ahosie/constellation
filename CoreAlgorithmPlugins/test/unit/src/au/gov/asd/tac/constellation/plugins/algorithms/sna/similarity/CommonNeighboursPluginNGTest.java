...
public class CommonNeighboursPluginNGTest {
    ...
    public void testCN() throws Exception {
        final CommonNeighboursPlugin instance = new CommonNeighboursPlugin();
        final PluginParameters parameters = instance.createParameters();
        parameters.setBooleanValue(CommonNeighboursPlugin.INCLUDE_CONNECTIONS_IN_PARAMETER_ID, true);
        parameters.setBooleanValue(CommonNeighboursPlugin.INCLUDE_CONNECTIONS_OUT_PARAMETER_ID, true);
        parameters.setBooleanValue(CommonNeighboursPlugin.TREAT_UNDIRECTED_BIDIRECTIONAL_PARAMETER_ID, true);
        parameters.setIntegerValue(CommonNeighboursPlugin.MINIMUM_COMMON_FEATURES_PARAMETER_ID, 1);
        parameters.setBooleanValue(CommonNeighboursPlugin.SELECTED_ONLY_PARAMETER_ID, false);
        parameters.setBooleanValue(CommonNeighboursPlugin.COMMUNITY_PARAMETER_ID, false);
        PluginExecution.withPlugin(instance).withParameters(parameters).executeNow(graph);

        assertEquals(graph.getFloatValue(transactionCNAttribute, 11), 3f);
        assertEquals(graph.getFloatValue(transactionCNAttribute, 18), 3f);
    }
}
