...
/**
 * Preferential Attachment Plugin Test.
 *
...
public class PreferentialAttachmentPluginNGTest {
    ...
    public void testPA() throws Exception {
        final PreferentialAttachmentPlugin instance = new PreferentialAttachmentPlugin();
        final PluginParameters parameters = instance.createParameters();
        parameters.setBooleanValue(PreferentialAttachmentPlugin.INCLUDE_CONNECTIONS_IN_PARAMETER_ID, true);
        parameters.setBooleanValue(PreferentialAttachmentPlugin.INCLUDE_CONNECTIONS_OUT_PARAMETER_ID, true);
        parameters.setBooleanValue(PreferentialAttachmentPlugin.TREAT_UNDIRECTED_BIDIRECTIONAL_PARAMETER_ID, true);
        parameters.setBooleanValue(PreferentialAttachmentPlugin.SELECTED_ONLY_PARAMETER_ID, false);
        PluginExecution.withPlugin(instance).withParameters(parameters).executeNow(graph);

        assertEquals(graph.getFloatValue(transactionPAAttribute, 11), 16f);
        assertEquals(graph.getFloatValue(transactionPAAttribute, 18), 16f);
    }
}
