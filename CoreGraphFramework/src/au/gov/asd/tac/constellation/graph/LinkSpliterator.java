 ...
 * @author algol
 */
public class LinkSpliterator implements Spliterator.OfInt {

    private final GraphReadMethods rg;
    private int position;
    private int limit;

    public LinkSpliterator(final GraphReadMethods rg) {
        this.rg = rg;
        position = 0;
        limit = rg.getLinkCount();
    }

    private LinkSpliterator(final GraphReadMethods rg, final int position, final int limit) {
        this.rg = rg;
    ...
    public OfInt trySplit() {
        ...
        return new LinkSpliterator(rg, limit, oldLimit);
    }

    @Override
    public boolean tryAdvance(final IntConsumer action) {
        if (position < limit) {
            final int vxId = rg.getLink(position++);
            action.accept(vxId);
            return true;
        }
